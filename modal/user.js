...
const update_users = async (userID,username, email, profile_pic_url, password) => {
    ...
    [hash, salt] = await generate_hash_password(password);
    [results, fields] = await dbConn.query(sql, [username, email, profile_pic_url, password, salt, userID]);
    return null

}

const login_user = async(username, password) => {
    [result, fields] = await dbConn.query("select password, salt from users where username = ?", [username]);
    if(results.length == 0) throw new Error("no such user");
        ...
        const match = await bcrypt.compare(given_hash, hash);
