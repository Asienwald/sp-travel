...
CREATE TABLE `reviews` (
  ...
  `fk_username` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`review_id`),
  KEY `fk_travel_id` (`fk_travel_id`),
  KEY `fk_username` (`fk_username`),
  CONSTRAINT `reviews_ibfk_3` FOREIGN KEY (`fk_travel_id`) REFERENCES `travel_listings` (`travel_id`) ON DELETE CASCADE,
  CONSTRAINT `reviews_ibfk_4` FOREIGN KEY (`fk_username`) REFERENCES `users` (`username`) ON DELETE CASCADE
);


INSERT INTO `reviews` (fk_travel_id, content, rating, fk_username) VALUES (1,'yo this is so good must try yoooooo',5,'imsorry');
INSERT INTO `reviews` (fk_travel_id, content, rating, fk_username) VALUES (1,'ok only la',3,'Pikachu');
INSERT INTO `reviews` (fk_travel_id, content, rating, fk_username) VALUES (2,'very cool! history much!!',5,'imsorry');
INSERT INTO `reviews` (fk_travel_id, content, rating, fk_username) VALUES (3,'had a bad time. do not recommend',1,'hahaha');
